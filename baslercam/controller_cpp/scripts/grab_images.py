...
camera_group_ids ={'40093166':'r', '40099899':'color', '40134758':'l'}

# #### 2. Define output folder


output_folder = "fhd_dataset/"

# #### 3. Open avena camera


print("Press 'o' to open cameras\nPress 's' to Save images \nPress 'c' to exit and Close cameras ")

idx = 0
def press(key):
    global avena_camera
    global images
    global camera_group_serials
    ...
    if key == "s":
        print("s is pressed")
        images = get_camera_group_images(avena_camera, output_folder)
        idx+=1
        for image in images:
            cv2.imwrite(list(image.keys())[0]+f"/image-{idx}.png", list(image.values())[0])
